...
#define DHTTYPE DHT11    //DHT11   // DHT 11
DHT dht(DHTPIN, DHTTYPE);

*/

float t, h, f, hic; //temperature, humidity, faranaith,healtemp.
...
void setup() {
    ...
    delay(2000);
    lcd.clear();
    // initialize the DHTxx temperature&Humidity
    dht.begin();
...
void dataLoggerSD() { //Store data in csv on sd card
    ...
    dataFile.println(dataString); //write the string as csv format on SD card file.
...
void tempHumLCD() {  //Send temp,humidity and Heatindex on LCD
